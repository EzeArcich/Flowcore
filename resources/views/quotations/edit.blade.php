    ...
    <div class="crm-page">
        <div class="crm-card max-w-3xl p-6 space-y-6">
            <header class="flex items-center justify-between">
                ...
                <a href="{{ route('companies.show', $quotation->company) }}" class="crm-link">Volver a la empresa</a>
            </header>
            ...
            <form method="POST" action="{{ route('quotations.update', $quotation) }}" class="space-y-4">
                ...
                <div>
                    ...
                    <input type="text" name="title" value="{{ old('title', $quotation->title) }}" class="crm-input" required>
                </div>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                    ...
                    <div>
                        ...
                        <select name="pricing_type" class="crm-select" required>
                ...
                <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                    <div>
                        ...
                        <input type="number" step="0.01" name="amount" value="{{ old('amount', $quotation->amount) }}" class="crm-input" required>
                    </div>
                    <div>
                        <label class="block text-sm font-medium mb-1">Moneda</label>
                        <input type="text" name="currency" value="{{ old('currency', $quotation->currency) }}" class="crm-input" required>
                    </div>
                    <div>
                        <label class="block text-sm font-medium mb-1">Horas estimadas</label>
                        <input type="number" name="estimated_hours" value="{{ old('estimated_hours', $quotation->estimated_hours) }}" class="crm-input">
                    </div>
                </div>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div>
                        ...
                        <input type="date" name="sent_at" value="{{ old('sent_at', optional($quotation->sent_at)->toDateString()) }}" class="crm-input">
                    </div>
                    <div>
                        <label class="block text-sm font-medium mb-1">Válida hasta</label>
                        <input type="date" name="valid_until" value="{{ old('valid_until', optional($quotation->valid_until)->toDateString()) }}" class="crm-input">
                    </div>
                </div>
                ...
                <div>
                    ...
                    <textarea name="scope_summary" rows="3" class="crm-textarea">{{ old('scope_summary', $quotation->scope_summary) }}</textarea>
                </div>

                <div>
                    <label class="block text-sm font-medium mb-1">Notas internas</label>
                    <textarea name="notes" rows="3" class="crm-textarea">{{ old('notes', $quotation->notes) }}</textarea>
                </div>

                <div class="flex items-center justify-end gap-3 pt-4">
                    <a href="{{ route('companies.show', $quotation->company) }}" class="crm-btn-secondary">Cancelar</a>
